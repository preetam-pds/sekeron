...
import { COLOR_HSVA, CONTRAST_RATIO, HEX_FORMAT, HSLA_FORMAT, HSL_FORMAT, HSL_HEX, HSVA_FORMAT, HSV_FORMAT, RGBA_FORMAT, RGB_FORMAT, } from './ColorsConversionFormulas';
...
export default function ColorPicker({
  ...
  const colorHash = useSharedValue(
    `${hue.current},${saturation.current},${brightness.current},${alpha.current}`,
  ...
  const setInverted = (
    {h = hue.current, s = saturation.current, b = brightness.current},
    color2,
  ) => {
  ...
  const updateBrightness = brightnessChannel => {
    ...
    const brightnessHandles = registeredHandles.current.filter(
      setting => setting.channel === 'b',
    );
  ...
  const updateSaturation = saturationChannel => {
    ...
    const saturationHandles = registeredHandles.current.filter(
      setting => setting.channel === 's',
    );
  ...
  const updateHue = hueChannel => {
    ...
    // update handles positions.
    const hueHandles = registeredHandles.current.filter(
      setting => setting.channel === 'h',
    );
  ...
  const updateOpacity = alphaChannel => {
    ...
    setInvertedColor();
    // update handles positions.
    const opacityHandles = registeredHandles.current.filter(
      setting => setting.channel === 'a',
    );
   ...
   */
  const applySettings = (
    settings = registeredHandles.current,
    withAnimation = true,
  ) => {
    const duration = 150;
    const color = {
    ...
    for (let i = 0; i < settings.length; i++) {
      ...
      if (axis === 'angle') {
        ...
        const posX =
          width -
          Math.round(Math.cos((color.h * Math.PI) / 180) * distance + center) -
          handleSize / 2;
        const posY =
          width -
          Math.round(Math.sin((color.h * Math.PI) / 180) * distance + center) -
          handleSize / 2;
  ...
  const registerHandle = settings => {
    ...
    const index = registeredHandles.current.findIndex(
      current => current.id && current.id === settings.id,
    );
    // update handle
    if (index !== -1) {
