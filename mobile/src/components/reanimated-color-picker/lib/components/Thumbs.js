...
export default function Thumb({ thumbShape = 'ring', thumbSize, thumbColor, handleStyle, vertical = false, channel }) {
    const { width, height, borderRadius } = { width: thumbSize, height: thumbSize, borderRadius: thumbSize / 2 };
    const { invertedColorHue, invertedColorBrightness, invertedColorSaturation, invertedColorOpacity, invertedColor, solidColor } = useContext(CTX);
    const inverted = channel === 'h'
        ? invertedColorHue
        : channel === 'b'
    ...
    const Ring = () => {
        ...
        return (<Animated.View style={[styles.handle, style, invertedStyle, handleStyle]}>
    ...
    const Line = () => {
        ...
        return (<Animated.View style={[styles.handle, style, handleStyle]}>
    ...
    const Plus = () => {
        ...
        return (<Animated.View style={[styles.handle, style, invertedBorderStyle, handleStyle]}>
    ...
    const Rect = () => {
        ...
        const invertedStyle = useAnimatedStyle(() => ({ borderColor: inverted.value }));
        return (<Animated.View style={[styles.handle, style, handleStyle]}>
        <Animated.View style={{width:15,height:30,borderColor:'#ffffff',borderWidth:2}}/>
      </Animated.View>);
    };
    const Circle = () => {
        const style = { width, height };
