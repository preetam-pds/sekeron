...
export function Swatches({ colors = SWATCHES_COLORS, style = {}, swatchStyle = {} }) {
    ...
    return (<View style={[styles.swatcheContainer, style]}>
      {colors.map((swatch, i) => (<Pressable key={swatch + i} onPress={() => onPress(swatch)} style={[styles.swatch, swatchStyle, { backgroundColor: swatch }]}/>))}
    </View>);
}
