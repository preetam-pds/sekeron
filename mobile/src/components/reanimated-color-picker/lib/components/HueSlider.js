...
export function HueSlider({ thumbShape, thumbSize, thumbColor, style = {}, vertical = false, reverse = false }) {
    const { registerHandle, updateHue, onGestureEventFinish, solidColor, sliderThickness, thumbSize: thumbsSize, thumbShape: thumbsShape, } = useContext(CTX);
    ...
    return (<PanGestureHandler onGestureEvent={gestureEvent} minDist={0}>
      <Animated.View onLayout={onLayout} style={[{ borderRadius }, vertical ? { width } : { height }, style, { position: 'relative', borderWidth: 0, padding: 0 }]}>
