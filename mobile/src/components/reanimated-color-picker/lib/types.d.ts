...
export declare type thumbShapeType = 'ring' | 'solid' | 'hollow' | 'line' | 'plus' | 'pill' | 'triangleUp' | 'triangleDown' | 'doubleTriangle' | 'rect' | 'circle';
export declare type THandleSettings = {
    id: string;
    channel: 'h' | 's' | 'b' | 'a';
    axis: 'x' | 'y' | 'angle';
    width: number;
...
export interface TCTX {
    ...
    initialColor: React.MutableRefObject<{
        h: number;
        s: number;
        b: number;
        a: number;
