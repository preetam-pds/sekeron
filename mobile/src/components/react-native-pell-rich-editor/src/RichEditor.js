...
const PlatformIOS = Platform.OS === PlatformEnum.ios;
...
export default class RichTextEditor extends Component {
  ...
  constructor(props) {
    ...
    that.state = {
      ...
      isTextEditorLoadCompleted: false,
    };
    that.focusListeners = [];
  }
  ...
  componentDidMount() {
    if (Platform.OS === 'android') {
      if (this.webviewBridge.current) this.webviewBridge.current.requestFocus();
    }
    this.unmount = false;
    if (PlatformIOS) {
  ...
  onMessage(event) {
    ...
    try {
      ...
      console.log(message.type, 'datadata');
      switch (message.type) {
